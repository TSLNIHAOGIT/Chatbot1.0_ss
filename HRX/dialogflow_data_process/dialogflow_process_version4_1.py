...
def id_dependency(all_others_name=None,update_input_data_speech=True):
...
13   请求更新金额    110
    ...
    for each in denpendency00:
        if each not in all_unique_labels:
            print('not exists',each)
    '''
    not exists IDClassifier107  ##
    not exists IfKnowDebtor0   *
    not exists ConfirmLoan1_2  *
    not exists ConfirmLoan108  ##
    not exists ConfirmLoan112
    not exists WillingToPay112
    
    '''
    not_exists=['IDClassifier107','ConfirmLoan108','ConfirmLoan112','WillingToPay112','',]


    if update_input_data_speech:
        for each in denpendency00:
            # if each in not_exists:
            #     continue
            # print('key each',each)

            # denpendency00[each]['input_data_speech']=huashu_df[huashu_df['label']==each,'message_finish'].tolist()
            denpendency00[each]['input_data_speech'] =huashu_df[huashu_df['label'] == each]['message_finish'].tolist()

    return denpendency00
...
#agent数据处理
def main_agent(path,cls_name=None,init=None,input_context=None,output_context=None,input_data_speech=None,fallbackIntent=False,**id):
...
if __name__=='__main__':
    ...
    for each_cls_name in cls_name_list:
        if each_cls_name=='init':
            ...
            # all_others_label = [102, 103, 104, 105, 106, 107, 108, 109, 110, 112]
            each_cls_others_label = list(df_others['label'].unique())
            each_cls_others_label.append(1000)
            print('each_cls_others_label',each_cls_name,each_cls_others_label)
            for each_label in each_cls_others_label:
    ...
    print('all_others_cls_name',all_others_cls_name)
    ...
    for each_cls_extend in cls_name_extend_list:
        ...
        label_num= each_cls_extend[-4:]
        # print('label_num',type(label_num),label_num)#label_num <class 'str'> 1000
